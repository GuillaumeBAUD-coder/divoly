...
export function GoogleAuthButton({ label = "Continue with Google" }: GoogleAuthButtonProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/providers")
      ...
      .then((providers: ProvidersResponse | null) => {
  ...
  return (
    <button
      ...
      className="flex w-full items-center justify-center gap-3 rounded-xl border border-white/15 bg-white px-4 py-3 text-sm font-semibold text-slate-950 transition hover:-translate-y-0.5 hover:bg-white/90 disabled:pointer-events-none disabled:opacity-60"
    >
      <GoogleMark />
